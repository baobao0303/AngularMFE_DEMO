...
export const mockApiInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  // 1. Dashboard API Mock Endpoints
  if (url.includes('/api/dashboard/projects')) {
    ...
    return of(new HttpResponse({ status: 200, body: mockData })).pipe(delay(300));
  }

  if (url.includes('/api/dashboard/benchmarks')) {
    const mockData = [
  ...
  if (url.includes('/api/dashboard/team-performance')) {
    const mockData = [
      {
        name: 'Sarah J.',
  ...
  if (url.includes('/api/dashboard/activity-logs')) {
    const mockData = [
      {
        id: '1',
        type: 'Projects',
  ...
  if (url.includes('/api/dashboard/kanban-tasks')) {
    const mockData = [
      {
        id: 'task-1',
  ...
  // 2. Reporting API Mock Endpoints
  if (url.includes('/api/reporting/detailed-reports')) {
    const mockData = [
      {
        id: 'REP-001',
  ...
  if (url.includes('/api/auth/login')) {
    ...
    const response = { user, token: `mock_jwt_${user.id}` };
    return of(new HttpResponse({ status: 200, body: response })).pipe(delay(400));
  }

  if (url.includes('/api/auth/sso-login')) {
    const user = {
  ...
  if (url.includes('/api/auth/reset-password')) {
    return of(new HttpResponse({ status: 200, body: { success: true, message: 'Password reset email sent' } })).pipe(delay(500));
  }
